...
function initFirebaseIfConfigured(){
    try{
        ...
        __FIREBASE.stateRef.onSnapshot((snap)=>{
            ...
            if(__STATE_CACHE){
                __STATE_CACHE.settings = __STATE_CACHE.settings || {};
                __STATE_CACHE.settings.isAdmin = isAdminNow();
            }
...
const db = {
    get: () => {
        ...
        if(__FIREBASE.enabled && __STATE_CACHE){
            __STATE_CACHE.settings = __STATE_CACHE.settings || {};
            __STATE_CACHE.settings.isAdmin = isAdminNow();
...
/* ============================
   HOTFIX PACK (Guide + Admin)
   - Adds missing app.openGuideEditor / app.deleteGuideItem
   - Makes admin logout reliable
   ============================ */
(function(){
  // Ensure window.app exists
  if(!window.app){ window.app = {}; }

  // Helper: get display name for logs
  function _actorName(){
    try{
      const u = (window.firebase && firebase.auth && firebase.auth().currentUser) ? firebase.auth().currentUser : null;
      return (u && (u.email || u.uid)) ? (u.email || u.uid) : "local";
    }catch(e){ return "local"; }
  }

  // GUIDE: Add/Edit via prompt() to avoid modal dependency
  if(typeof window.app.openGuideEditor !== "function"){
    window.app.openGuideEditor = async function(id){
      try{
        const data = db.get();
        if(!data || !data.settings || !data.settings.isAdmin){
          util.notify("Not admin");
          return;
        }
        data.guide = Array.isArray(data.guide) ? data.guide : [];
        const existing = id ? data.guide.find(x => x && x.id === id) : null;

        const title = prompt("Guide title:", existing ? (existing.title||"") : "");
        if(title === null) return;

        const content = prompt("Guide content:", existing ? (existing.content||"") : "");
        ...
        if(existing){
          ...
          existing.by = _actorName();
          db.save(data, "GUIDE_EDIT");
          util.notify("Guide updated");
        } else {
          const newId = "g_" + Math.random().toString(36).slice(2,10) + "_" + now;
          data.guide.unshift({
            id: newId,
            title,
            content,
          ...
          db.save(data, "GUIDE_ADD");
          util.notify("Guide added");
        }
        window.app.render && window.app.render();
      }catch(e){
        console.error(e);
        alert("Guide editor error: " + (e && e.message ? e.message : e));
      }
    };
  }

  if(typeof window.app.deleteGuideItem !== "function"){
    window.app.deleteGuideItem = async function(id){
      try{
        const data = db.get();
        if(!data || !data.settings || !data.settings.isAdmin){
          util.notify("Not admin");
        ...
        data.guide = Array.isArray(data.guide) ? data.guide : [];
        data.guide = data.guide.filter(x => x && x.id !== id);
        db.save(data, "GUIDE_DELETE");
        util.notify("Deleted");
        window.app.render && window.app.render();
      }catch(e){
        console.error(e);
  ...
  // ADMIN: Make logout always clear local + ui state + firebase session
  if(typeof window.app.forceAdminLogout !== "function"){
    window.app.forceAdminLogout = async function(){
      try{
        // Clear local admin flags if any
        try{ localStorage.removeItem("USL_LOCAL_ADMIN"); }catch(e){}
        // Clear UI admin flag
        const data = db.get();
        if(data && data.settings){ data.settings.isAdmin = false; db.save(data, "ADMIN_LOGOUT"); }
        // Firebase signout if present
        ...
        try{ await refreshAdminCache(true); }catch(e){}
        util.notify("Logged out");
        window.app.render && window.app.render();
      }catch(e){
        console.error(e);
        alert("Logout error: " + (e && e.message ? e.message : e));
      }
    };
  }

  // If your UI uses app.toggleAdmin(), wrap it so it can always log out
  if(typeof window.app.toggleAdmin === "function" && !window.app._toggleAdminWrapped){
    const _old = window.app.toggleAdmin;
    window.app.toggleAdmin = async function(){
      const data = db.get();
      if(data && data.settings && data.settings.isAdmin){
        return window.app.forceAdminLogout();
      }
      return _old.apply(this, arguments);
    };
    window.app._toggleAdminWrapped = true;
  }
})();
